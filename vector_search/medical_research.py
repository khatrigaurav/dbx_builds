...
MAX_PAPERS = 150  # Number of papers to download per query
...
# Configuration
EUROPEPMC_API_URL = "https://www.ebi.ac.uk/europepmc/webservices/rest/search"
SEARCH_QUERY = " covid AND treatment"  # Modify this to search for different topics
MAX_PAPERS = 150  # Number of papers to download per query

# Output directory - Unity Catalog volume path (for Databricks)
# For local testing, use: VOLUME_PATH = "./data/medical_papers"
VOLUME_PATH = "/Volumes/gaurav_catalog/medical_research/raw_papers"  # ← Note the leading /
...
"""
            ...
            try:
                # Try standard file I/O first (works on DBR 13.3+)
                ...
                # Fallback to dbutils.fs for older runtimes or volume issues
                if idx == 1:  # Only print once
                    print(f"  ℹ️  Using dbutils.fs (direct file I/O not supported)")
                
                try:
                    dbutils.fs.put(text_filename, content, overwrite=True)
                    dbutils.fs.put(json_filename, json.dumps(paper, indent=2), overwrite=True)
                except NameError:
                    # dbutils not available (running locally)
                    raise IOError(f"Cannot write to {output_path}. If running in Databricks, ensure volume exists and you have write permissions.") from io_error
        ...
        try:
            ...
            # Fallback to dbutils.fs
            try:
                dbutils.fs.put(summary_filename, json.dumps(summary, indent=2), overwrite=True)
            except NameError:
                pass  # Skip summary if can't write
...
df_bronze = df_with_content.select(
    col("file_name"),
    col("file_path"),
...
# Retry endpoint status fetch until KeyError is not raised
import time
...
for attempt in range(max_retries):
    try:
        ...
        print(f"KeyError encountered: {e}. Retrying ({attempt+1}/{max_retries})...")
        time.sleep(retry_delay)
else:
    print("Failed to get endpoint status after multiple retries.")
...
print(f"   - Index Type: DIRECT_ACCESS ")
print(f"\n🎯 NEXT STEPS:")
...
#   query_text = "covid treatments side effects",
#   columns=["chunk_id", "file_name", "chunk_text", "chunk_sequence"],
#   num_results=2,
...
index.similarity_search(
  query_vector = [0.19]*1024,
  query_text = "side effects of mrna",
  columns=["chunk_id", "file_name", "chunk_text", "chunk_sequence"],
  num_results=3,
  query_type="FULL_TEXT"
)